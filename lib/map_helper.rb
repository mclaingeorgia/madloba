module MapHelper
  ...
  MAP_SERVICE_ATTRIBUTES = [:mapbox_api_key, :mapbox_map_english_name, :mapbox_map_georgian_name, :mapbox_tile_url, :mapbox_attribution, :mapquest_api_key, :osm_tile_url, :osm_attribution]
  ...
  def init_map_settings
    ...
    %w(api_key map_english_name map_georgian_name attribution).each {|key| self.send("mapbox_#{key}=", @mapbox.send(key))}
