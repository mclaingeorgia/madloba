...
namespace :uploader_v3 do

  original_dataset_path = "#{Rails.root}/public/data/uploader_v2/dataset.csv"
  cleaned_dataset_path = "#{Rails.root}/public/data/uploader_v2/cleaned_dataset.csv"
  missing_dataset_path = "#{Rails.root}/public/data/uploader_v2/missing_geocode.csv"
  ...
  task import_data: :environment do
    ...
    CSV.foreach(cleaned_dataset_path, headers: true) do |row, row_i|
      ...
      provider_name_en = clean_string_from_spaces(row[1])
        ...
        place_director_ka = ''
        place_director_en = ''
        place_director = row[8]
        if !place_director.empty?
          place_director_ka = clean_string_from_spaces(place_director)
        end
        place_director_en = latinize(place_director_ka)

        phones = row[11].gsub(' ', '').gsub('-', '').gsub('"', '').gsub('.', '').gsub(',', '').chomp(';').split(';')
        emails = row[10].gsub(' ', '').split(';')
        websites = row[9].gsub(' ', '').split(';')
        facebook, website = nil
        websites.each do |url|
          if url.include?('facebook.com') && facebook.nil?
            facebook = url
          elsif website.nil?
            website = url
          end
        ...
        place = {
          phone: phones.present? ? phones[0] : nil,
          phone2: phones.present? && phones[1].present? ? phones[1] : nil,
          email: emails.present? ? emails[0] : nil,
          website: website,
          facebook: facebook,
          latitude: row[15].to_f,
          longitude: row[16].to_f,
