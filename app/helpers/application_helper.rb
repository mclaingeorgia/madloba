module ApplicationHelper
  ...
  def getMapSettingsWithSeveralItems(ad, hasCenterMarker, clickableMapMarker)
    ...
    if ad.location.is_area
      ...
      @map_settings['popup_message'] = categories_to_show.join(', ')
    else
      ...
      ad.categories.each_with_index do |category, index|
        ...
        @map_settings['ad_show'][index]['icon'] = category.icon
        @map_settings['ad_show'][index]['color'] = category.marker_color
        @map_settings['ad_show'][index]['item_name'] = category.name
      end
  ...
  def getMapSettings(location, hasCenterMarker, clickableMapMarker)
    ...
    if I18n.locale == :en
      api_keys['mapbox_map_id'] = @map_settings['english_map']
    elsif I18n.locale == :ka
      api_keys['mapbox_map_id'] = @map_settings['georgian_map']
    end
