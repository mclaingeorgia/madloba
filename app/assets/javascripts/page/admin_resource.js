  ...
  var resource_list = {
    ...
    bind: function () {
      ...
      t.el.on('click', '> li .delete', function (event) {
        ...
        if(!is_virtual) {
          list.append('<input type="hidden" value="1" name="resource[resource_items_attributes][' + index + '][_destroy]">')
        }
        else {
          --virtual_length
        }
        wrapper.find('.orders [name="resource[resource_items_attributes][' + index + '][order]"]').remove()
      ...
      $('#add_resource_list_item').click(function (event) {
        ...
        wrapper.find('.orders').append('<input class="field-input hidden" type="hidden" value="' + (ln+1) + '" name="resource[resource_items_attributes][' + ln + '][order]">')
      ...
      function reorderItems(before, after) {
        wrapper.find('[data-link="' + gon.locales[0] + '"] ul li').each(function (i, d) {
          ...
          wrapper.find('.orders [name="resource[resource_items_attributes][' + index + '][order]"]').val(i+1)
        })
