...
$(document).ready(function(){
  ...
  var flash_el = $("#flash")
  var flash_messages = flash_el.find('.messages')
  ...
  $(document).ajaxComplete(function(event, request) {
    ...
    if(request.hasOwnProperty('responseJSON') && request.responseJSON.hasOwnProperty('flash')) {
      flash(request.responseJSON.flash)
      console.log('ajaxComplete flash sended')
    }
  });

  flash_el.find('.close').on('click', function (event) {
    flash_el.removeAttr('open')
    flash_messages.empty()
  })

  if(flash_el.find('.messages li').length) {
    flash_el.delay(5000).fadeOut(2000, function () {
      flash_el.removeAttr('open')
      flash_el.css('display', '')
      flash_messages.empty()
    })
  }
  ...
  function flash(messages)
  {
    ...
    if(messages.length) {
      messages.forEach(function(message) {
        html += "<li class='message'><div class='flag " +  message.type + "'></div><div class='text'>" +urldecode(message.text) + "</div></li>"
      })
    }

    flash_messages.html(html)
    flash_el.attr('open', 'open').delay(5000).fadeOut(2000,function(){
      flash_el.removeAttr('open')
      flash_el.css('display', '')
      flash_messages.empty()
    });
